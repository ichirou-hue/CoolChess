...
export function AuthPage() {
  ...
  const switchMode = (nextMode: AuthMode) => { setMode(nextMode); setSubmitted(false); window.history.replaceState(null, '', `#auth/${nextMode}`); };
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSubmitted(true); localStorage.setItem('coolchess.authDemo', JSON.stringify({ authenticated: true, mode })); };
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f7f7fb] text-[#111]">
    <div className="grid min-h-screen lg:grid-cols-[minmax(420px,0.78fr)_minmax(520px,1.22fr)]">
      <section className="relative hidden overflow-hidden bg-[#464bff] p-10 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative z-10 max-w-md"><span className="mb-5 inline-flex rounded-full bg-white/15 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em]">Шахматная школа</span><h1 className="text-6xl font-extrabold leading-[0.94] tracking-[-0.06em]">Твой ход<br /><span className="text-[#dce204]">к сильной игре.</span></h1><p className="mt-7 max-w-sm text-sm leading-7 text-white/75">Сохраняй прогресс, проходи темы по порядку и возвращайся к задачам каждый день.</p></div>
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-10"><div className="w-full max-w-[470px]">
        ...
        <div className="mb-8"><span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#464bff]">{isRegister ? '01 · СОЗДАНИЕ ПРОФИЛЯ' : '01 · ВОЗВРАЩЕНИЕ В ИГРУ'}</span><h2 className="mt-4 text-4xl font-extrabold leading-none tracking-[-0.05em] sm:text-5xl">{isRegister ? <>Создай свой<br /><span className="text-[#464bff]">маршрут.</span></> : <>С возвращением<br /><span className="text-[#464bff]">в CoolChess.</span></>}</h2><p className="mt-5 text-sm leading-6 text-[#777985]">{isRegister ? 'Один аккаунт — теория, задачи, серия и рейтинг в одном месте.' : 'Войди, чтобы продолжить обучение с того места, где остановился.'}</p></div>
        ...
        {submitted && <div className="mb-5 rounded-xl border border-[#bde5c8] bg-[#effbea] px-4 py-3 text-xs font-semibold text-[#227a3d]">Демо-форма готова. После подключения API здесь появится реальная авторизация.</div>}
        <form className="space-y-4" onSubmit={submit}>
          {isRegister && <label className="block"><span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-[#777985]">Имя ученика</span><input required name="name" type="text" placeholder="Например, Егор" className="h-13 w-full rounded-xl border border-[#e1e2eb] bg-white px-4 text-sm outline-none transition placeholder:text-[#b6b7c1] focus:border-[#464bff] focus:ring-4 focus:ring-[#464bff]/10" /></label>}
          ...
          <button type="submit" className="h-13 w-full rounded-xl bg-[#464bff] px-5 text-sm font-bold text-white shadow-[0_12px_24px_rgba(70,75,255,.22)] transition hover:-translate-y-0.5 hover:bg-[#363be8]">{isRegister ? 'Создать профиль' : 'Войти в CoolChess'} <span className="ml-2">↗</span></button>
        </form>
        ...
        <p className="mt-10 text-center text-[10px] leading-5 text-[#b0b1bb]">Пока работает демонстрационный режим. Данные формы не отправляются на сервер.</p>
      </div></section>
    </div>
  </main>;
}
