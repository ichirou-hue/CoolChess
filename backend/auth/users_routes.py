...
from auth.schemas import LichessSyncRequest, LichessSyncResponse
...
@users_router.post("/api/users/sync-lichess", response_model=LichessSyncResponse, tags=["Users"])
async def sync_lichess_account(
    ...
    """
    Синхронизирует профиль с Lichess: подтягивает актуальные рейтинги
    и калибрует стартовый Elo новичка (если еще не сыграно матчей).
    """
    ...
    # Калибровка стартового рейтинга (если игрок новичок: 0 игр и дефолтный Elo 1200)
    ...
    return LichessSyncResponse(
        ...
        message=f"Аккаунт Lichess {current_user.lichess_username} успешно привязан!",
    )